...
type ScoutContract = {
  ...
  codeHash: string; codeSize: number; risk: ScoutRisk; token: null | { name: string | null; symbol: string | null; decimals: number | null; totalSupply: string | null; owner: Address | null; paused: boolean | null; isNFT?: boolean; tokenKind?: string | null };
...
type ScoutTokenMeta = { name: string | null; symbol: string | null; decimals: number | null; totalSupply: string | null; owner: Address | null; paused: boolean | null; tokenLike?: boolean; tokenKind?: string | null; isNFT?: boolean }
...
function renderScoutContract(item: ScoutContract): string {
  ...
  const subtitle = item.token ? `${item.token.name ?? 'ERC-20-like token'}${item.token.isNFT ? ' · 🎨 NFT' : ''} · ${item.token.decimals ?? '?'} decimals` : `${item.codeSize.toLocaleString()} byte runtime`
